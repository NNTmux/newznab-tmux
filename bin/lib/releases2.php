...
{	
	...
	function processReleases() 
	{
						...
						if ($regexrow["categoryID"] != "")
							...
							if ($regcatid == Category::CAT_PC_0DAY)
							{
								...
								if ($cat->isEBook($matches['name']))
									$regcatid = Category::CAT_MISC_EBOOK;
							}
		...
		while ($row = mysql_fetch_assoc($result)) 
		{
			...
			{
				echo "Stage 5 : Failed to write nzb file (bad perms?) ".$nzbfile."\n";
		...
		echo "Stage 4 : Finished creating ".$retcount." releases. Found ".$nfocount." nfos\n";
		...
		$db->query(sprintf("DELETE parts, binaries FROM parts LEFT JOIN binaries ON binaries.ID = parts.binaryID LEFT JOIN releasenfo rn ON
								rn.binaryID = binaries.ID WHERE (binaries.procstat IN (%d, %d) AND 
								(rn.binaryid IS NULL OR (rn.binaryid IS NOT NULL AND rn.nfo IS NOT NULL))) OR
								binaries.dateadded < %s - INTERVAL %d HOUR", Releases::PROCSTAT_RELEASED, Releases::PROCSTAT_DUPLICATE, $db->escapeString($currTime_ori["now"]), ceil($page->site->rawretentiondays*24)));
