...
class PostProcess4
{
	...
	public function processAdditional4()
	{
		...
		$result = $db->query(sprintf("select r.ID, r.guid, r.name, c.disablepreview from releases r 
			...
			or (r.haspreview = -1 and c.disablepreview = 0) order by r.guid desc  limit %d, %d ", ($maxattemptstocheckpassworded + 1) * -1, 2.5 * $numtoProcess, $numtoProcess ));
		
		$iteration = $rescount = sizeof($result);
		if ($rescount > 0)
		{
			echo "Post processing by guid on ".$rescount." releases, starting at 250 ...";		
			$nntpconnected = false;
