...
$usptotalconnections  = str_replace("\n", '', shell_exec("ss -n | grep -c " . $ip . ":" . $port));
    ...
    */
	...
	if (( $array['MAX_LOAD'] >= get_load()) && ( $movie_releases_proc > 0 )) {
		...
		shell_exec("$_tmux respawnp -t {$array['TMUX_SESSION']}:2.2 'echo \"\033[38;5;\"$color\"\" && $ds1 $panes2[2] $ds2 && $_python ${DIR}/../test/postprocess_threaded.py movie 2>&1 $log && echo \" \033[1;0;33m\" && $ds1 $panes2[2] $ds3' 2>&1 1> /dev/null");
	} elseif  ( $array['MAX_LOAD'] <= get_load()) {
                $color = get_color();
