...
class Backfill 
{
	...
	function backfillAllGroups($groupName='', $groupPost='', $backfillDate=null, $backfillPost=null)
	{
		...
		if ($res)
		{
			...
			if ($nntp->doConnect()) {
				$nntpt = new Nntp();
				$nntpt->doConnect($comp=true);
				foreach($res as $groupArr)
				{
					$this->backfillGroup($nntp, $nntpt, $groupArr, $backfillDate, $backfillPost=$groupPost);
				}
	 ...
	 */	
	function backfillGroup($nntp, $nntpt, $groupArr, $backfillDate=null, $backfillPost=null)
	{
		$db = new DB();
		...
		$datat = $nntpt->selectGroup($groupArr['name']);
		...
		while($done === false)
		{
			...
			flush();
			$success = $binaries->scan($nntpt, $groupArr, $first, $last, 'backfill');
			if (!$success)
			{
                $db->rollback();
				return "";
