...
$ps->maxChildren = 24;
$ps->tickCount = 5000;	// value in usecs. change this to 1000000 (one second) to reduce cpu use
...
//			echo "[Thread-MASTER] Spawning new thread.  Still have " . count($theList) ." posts to sort after this\n";
	...
	else
	{
		...
		echo "No more to process - Initiating shutdown\n";
		$ps->Shutdown();
...
//	echo "[Thread-{$thread}] Begining sorting for {$post['name']}\n";
...
//	echo "[Thread-{$thread}] Completed update for {$post['name']}\n";
	$tim = microtime(true) - $time;
//	echo "time = ".$tim."\n";
}
