...
function domatching2 ($pattern, $db, $r, $oldname, $sect, $parts = -1)
{
	global $notyet, $jump;
...
//trigger_error("CR= ".count($results)." AR= ".count($arr)." missing= $missing age= ".$r['age']." l= $l s= $s P=$parts ".$r['ID']);
		...
		echo "CR= ".count($results)." AR= ".count($arr)." missing= $missing age= ".$r['age']." l= $l s= $s\n";
			...
			} else if ($missing <= 0 && $l < $s && $r['age'] > .25 && count($results) == count($arr) && count($results) > 3 && $parts == -1)
			{
				$jump = $jump && makerelease($results, $r, $name, $db, $sect, $mail[1]);
			} else if ($missing <= 0 && $r['age'] > 2.5 && count($results) == count($arr) && count($results) > 3 && $parts == -1) {
				$jump = $jump && makerelease($results, $r, $name, $db, $sect, $mail[1]);
			}  else if ($r['age'] > .5 && count($results) == count($arr) && count($results) == 1 && $parts == 1) {
				$jump = $jump && makerelease($results, $r, $name, $db, $sect, $mail[1]);
			} else if ($r['age'] > .5 && count($results) == count($arr) && $parts == 0) {
				$jump = $jump && makerelease($results, $r, $name, $db, $sect, $mail[1]);
			} else
				$jump = $jump && clearmatches('', $results);
		}
...
//WHERE  `name` REGEXP CONCAT(  '[\{\[\(]0*1\/',  `totalParts` ,  '[\]\}\)]$' ) AND  `totalParts` >10
	...
	//			domatching($pattern, $db, $r, $oldname, "B");
			...
			$pattern = '/(?|(\d{1,2}?-\d{1,2}?\-(?:19|20)\d\d)|((?:19|20)\d\d\-\d{1,2}?-\d{1,2}?)|(.+\d{1,2}.[a-z]{3,4}.(?:19|20)\d\d))/iU';
				if (preg_match($pattern, $r['name'])  && $notyet)
					echo "name to match X: ".$r['name']."\n";
				domatching2($pattern, $db, $r, $oldname, "X", 0);
