...
//		var_dump($ax);
			...
			if (isset($ax["@attributes"]))
				...
				$a = $ax;
			if ((isset($title[2]) && $a["Year"] == $title[2]) || !isset($title[2]))
			{
				$i = 0;
...
//						var_dump($tmp);
	...
	if (preg_match('/^[a-f0-9]+$/', $thename))
	{
		if (isset($set[1]))
			$thename = $set[1];
		else if (preg_match('/[a-f0-9][ .]/', $name1))
			$thename = $name1;
		$thename = preg_replace('/\.\_/', ' ', 	$thename);
