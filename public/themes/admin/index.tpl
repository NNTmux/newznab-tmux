...
<div style="{if $site->showadminwelcome != "1"}display:none;{/if}" id="adminwelcome">
	...
	<li style="margin-bottom: 15px;">Next you will want to get the latest headers. <b>This should be done from the command line</b>, using the linux or windows shell scripts found in /misc/update (or win for windows users), as it can take some time. If this is your first time dont bother with the init scripts just open a command prompt...
	<div style="padding-left:20px; font-family:courier;">cd /var/www/newznab/misc/update<br/>php update_binaries.php</div>
	</li>
	<li style="margin-bottom: 15px;">After obtaining headers, the next step is to create releases. <b>This is best done from the command line</b> using the linux or windows shell scripts found in /misc/update (or win for windows users). If this is your first time dont bother with the init scripts just open a command prompt...
