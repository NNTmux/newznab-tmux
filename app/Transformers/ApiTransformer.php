    ...
    public function transform(\stdClass $releases): array
    {
        if (\in_array($releases->categories_id, Category::MOVIES_GROUP, false)) {
            return [
                ...
                'imdbid' => $releases->imdbid !== 0 ? $releases->imdbid : $this->null(),
                'tmdbid' => $releases->tmdbid !== 0 ? $releases->tmdbid : $this->null(),
                'traktid' => $releases->traktid !== 0 ? $releases->traktid : $this->null(),
        ...
        if (\in_array($releases->categories_id, Category::TV_GROUP, false)) {
            return [
                ...
                'tvdbid' => $releases->tvdb !== 0 ? $releases->tvdb : $this->null(),
                'traktid' => $releases->trakt !== 0 ? $releases->trakt : $this->null(),
                'tvrageid' => $releases->tvrage !== 0 ? $releases->tvrage : $this->null(),
                'tvmazeid' => $releases->tvmaze !== 0 ? $releases->tvmaze : $this->null(),
                'imdbid' => $releases->imdb !== 0 ? $releases->imdb : $this->null(),
                'tmdbid' => $releases->tmdb !== 0 ? $releases->tmdb : $this->null(),
                'grabs' => $releases->grabs !== 0 ? $releases->grabs : $this->null(),
                'comments' => $releases->comments !== 0 ? $releases->comments : $this->null(),
                'password' => $releases->passwordstatus,
                'usenetdate' => Carbon::parse($releases->postdate)->toRssString(),
