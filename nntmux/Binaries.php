...
class Binaries
{
	...
	public function scan($groupMySQL, $first, $last, $type = 'update', $missingParts = null)
	{
		...
		foreach ($headers as $header) {
			...
			if (preg_match('/^\s*(?!"Usenet Index Post)(.+)\s+\((\d+)\/(\d+)\)/', $header['Subject'], $matches)) {
				...
				if (strpos($header['Subject'], 'yEnc') !== false) {
					$matches[1] .= ' yEnc';
				}
			} else {
			...
			if (!isset($header['Bytes'])) {
				$header['Bytes'] = (isset($header[':bytes']) ?? $header[':bytes']);
			}
	...
	public function postdate($post, array $groupData)
	{
		...
		do {
			...
			if (!$this->_nntp->isError($header) && isset($header[0]['Date'])) {
				// Check if the date is set.
				if (strlen($header[0]['Date']) > 0) {
					$date = $header[0]['Date'];
					break;
				}
			}
		...
		return $this->_pdo->query(
			sprintf('
				SELECT
					...
					bb.groupname AS groupname, bb.regex, g.id AS groups_id, bb.msgcol,
