...
class Releases
{
    ...
    public function search($searchName, $usenetName, $posterName, $fileName, $groupName, $sizeFrom, $sizeTo, $hasNfo, $hasComments, $daysNew, $daysOld, $offset = 0, $limit = 1000, $orderBy = '', $maxAge = -1, array $excludedCats = [], $type = 'basic', array $cat = [-1], $minSize = 0): array
    {
        ...
        $baseSql = sprintf(
            "SELECT r.id, r.searchname, r.size, r.fromname, r.adddate, r.groups_id, r.grabs, r.passwordstatus, r.guid, r.completion, r.categories_id, r.comments, r.postdate, r.totalpart, r.videos_id, r.tv_episodes_id, r.videostatus, r.imdbid, r.anidbid, r.nfostatus,
				CONCAT(cp.title, ' > ', c.title) AS category_name,
				%s AS category_ids,
				df.failed AS failed,
				g.name AS group_name,
