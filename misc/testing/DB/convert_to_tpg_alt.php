...
if ($collections_rows instanceof \Traversable) {
	foreach ($collections_rows as $row) {
		...
		//binaries
		$pdo->queryExec("INSERT IGNORE INTO binaries_${row['groups_id']} (name, filenumber, totalparts, currentparts, binaryhash, partcheck, partsize, collection_id) "
		...
		$binaries = $pdo->queryOneRow("SELECT COUNT(*) AS cnt FROM binaries b INNER JOIN collections c ON  b.collection_id = c.id where c.groups_id = ${row['groups_id']}");
		...
		//parts
		$pdo->queryExec("INSERT IGNORE INTO parts_${row['groups_id']} (messageid, number, partnumber, size, binaryid, collection_id) "
			...
			. "INNER JOIN collections_${row['groups_id']} c on c.id = n.collection_id AND c.groups_id = ${row['groups_id']}");
		$parts = $pdo->queryOneRow("SELECT COUNT(*) AS cnt FROM parts p INNER JOIN binaries b ON p.binaryid = b.id INNER JOIN collections c ON b.collection_id = c.id WHERE c.groups_id = ${row['groups_id']}");
