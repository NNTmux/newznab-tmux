...
while ($cdone < $clen['total']) {
    ...
    if ($collections instanceof \Traversable) {
        foreach ($collections as $collection) {
            ...
            $newcid = ['collections_id' => $pdo->queryInsert(sprintf('INSERT INTO collections_%d (subject, fromname, date, xref, totalfiles, groups_id, collectionhash, dateadded, filecheck, filesize, releaseid) VALUES (%s);', $collection['groups_id'], implode(', ', $collection)))];
        ...
        while ($pdone < $plen['total']) {
            ...
            if ($partrepairs instanceof \Traversable) {
                foreach ($partrepairs as $partrepair) {
                    ...
                    $pdo->queryExec(sprintf('INSERT INTO partrepair_%d (numberid, groups_id, attempts) VALUES (%s, %s, %s);', $group['id'], $partrepair['numberid'], $partrepair['groups_id'], $partrepair['attempts']));
...
$pdo->queryExec('UPDATE tmux SET value = 1 where setting = \'releases\';');
