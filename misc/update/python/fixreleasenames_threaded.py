...
if len(sys.argv) > 1 and sys.argv[1] == "nfo":
	...
	run = "SELECT id AS prehashid FROM predb WHERE LENGTH(title) >= 15	AND title NOT REGEXP '[\"\<\> ]' AND searched = 0 AND DATEDIFF(NOW(), predate) > 1 ORDER BY predate ASC LIMIT %s"
	cur[0].execute(run, (int(perrun[0]) * int(run_threads[0])))
	datas = cur[0].fetchall()
