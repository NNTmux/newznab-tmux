...
switch ($argv[1]) {
	case 'sphinx':
		if (NN_RELEASE_SEARCH_TYPE == ReleaseSearch::SPHINX) {
			...
			echo PHP_EOL . $this->c->error('Error, NN_RELEASE_SEARCH_TYPE in www/settings.php must be set to SPHINX to optimize for Sphinx!' . PHP_EOL);
		}
		break;
	case 'standard':
...
{
	echo PHP_EOL . $this->c->info('Dropping search triggers to save CPU and lower QPS. (Quick)' . PHP_EOL);
	dropSearchTriggers($pdo);

	echo $this->c->info('Truncating releasesearch table to free up memory pools/buffers.  (Quick)' . PHP_EOL);
	$pdo->queryExec('TRUNCATE TABLE releasesearch');

	echo $this->c->header('Optimization for Sphinx process complete!' . PHP_EOL);
}
...
function revertToStandard($pdo)
{
	...
	echo PHP_EOL . $this->c->info('Dropping old table data and recreating fresh from schema. (Quick)' . PHP_EOL);
	...
	echo $this->c->info('Populating the releasearch table with initial data. (Slow)' . PHP_EOL);
	$pdo->queryInsert('INSERT INTO releasesearch (releaseID, guid, name, searchname)
				SELECT ID, guid, name, searchname FROM releases');

	echo $this->c->info('Adding the auto-population triggers. (Quick)' . PHP_EOL);
	...
	echo $this->c->header('Standard search should once again be available.' . PHP_EOL);
}
