...
if (isset($argv[1]) && $argv[1] === "all") {
	$titles = $pdo->queryDirect("SELECT id AS preid, title, source, searched FROM prehash
...
//Selects PreDB Titles where predate is greater than the past user selected number of hours
	...
	$titles = $pdo->queryDirect(sprintf("SELECT id AS preid, title, source, searched FROM prehash
						 WHERE LENGTH(title) >= 15 AND searched = 0
...
if ($total > 1) {
	...
	if ($titles instanceof \Traversable) {
		foreach ($titles as $row) {
			...
			$pdo->queryExec(sprintf("UPDATE prehash SET searched = %d WHERE id = %d", $searched, $row['preid']));
			if (!isset($argv[2]) || $argv[2] !== 'show') {
