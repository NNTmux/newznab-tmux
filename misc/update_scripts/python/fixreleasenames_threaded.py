...
if len(sys.argv) > 1 and sys.argv[1] == "nfo":
	...
	run = "SELECT DISTINCT rel.id FROM releases rel LEFT OUTER JOIN releasefiles rf ON rel.id = rf.releaseid AND rf.ishashed = 1 WHERE nzbstatus = 1 AND rel.dehashstatus BETWEEN -6 AND 0 AND rel.ishashed = 1 AND prehashid = 0 ORDER BY dehashstatus DESC, postdate ASC LIMIT %s"
	cur[0].execute(run, (int(perrun[0])*int(run_threads[0])))
	datas = cur[0].fetchall()
