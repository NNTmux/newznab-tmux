					...
					<td class="item">
						<label for="chk{$result.guid|substr:0:7}">
							<a class="title" title="View details"
							   href="{$smarty.const.WWW_TOP}/details/{$result.guid}/{$result.searchname|escape:"htmlall"}">{$result.searchname|escape:"htmlall"|truncate:150:"...":true}</a>{foreach from=$failed item=fail}{if $fail > 0} <i class="fa fa-exclamation-circle" style="color: red" title="This release has failed to download for some users"></i>{/if}{/foreach}</label value="Searchname">
						<div class="resextra">
							<div class="btns" style="float:right">
								...
								{/if}
								{foreach from=$failed item=fail}
									{$fail|@var_dump}
								{if $fail > 0}<span class="label label-default">
									<i class ="fa fa-thumbs-o-up"></i> {$result.grabs} Grab{if $result.grabs != 1}s{/if} / <i class ="fa fa-thumbs-o-down"></i> {$fail} Failed Download{if $fail != 1}s{/if}</span>
								{/if}
								{/foreach}
							</div>
						</div>
