...
<table style="width:100%;" class="data table table-striped" id="browsetable">
	...
	{foreach $results as $result}
		<tr class="{cycle values=",alt"}">
			...
			{if $isadmin}
			<td><span title="procstat">{$result.procstat}</span>/<span title="totalparts">{$result.totalParts}</span>/<span title="regex">{if $result.regexideq""}_{else}{$result.regexid}{/if}</span>/<span title="relpart">{$result.relpart}</span>/<span title="reltotalpart">{$result.reltotalpart}</span></td>
