...
<table style="width:100%;" class="data table" id="browsetable">
	...
	{foreach $results as $result}
		<tr class="{cycle values=",alt"}">
			...
			{if isset($isadmin)}
			<td><span title="procstat">{$result.procstat}</span>/<span title="totalparts">{$result.totalParts}</span>/<span title="relpart">{$result.relpart}</span>/<span title="reltotalpart">{$result.reltotalpart}</span></td>
