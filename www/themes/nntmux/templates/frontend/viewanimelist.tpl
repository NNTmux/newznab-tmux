...
<div class="pagination pagination-mini">
	<ul style="padding-bottom:1.5px;">
		<li{if $animeletter == '0-9'} class="active"{/if}>
			<a href="{$smarty.const.WWW_TOP}/anime/0-9">0-9</a>
		</li>
		{foreach $animerange as $range}
			<li{if $range == $animeletter} class="active"{/if}>
				<a href="{$smarty.const.WWW_TOP}/anime/{$range}">{$range}</a>
			</li>
		{/foreach}
	</ul>
	<form style="float:right;" name="anidbsearch" action="" method="get">
			<input id="title" type="text" value="{$animetitle|escape:'html'}" />
