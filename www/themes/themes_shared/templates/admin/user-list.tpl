...
<table style="width:100%;margin-top:10px;" class="data highlight">
	...
	<tr>
		...
		<th class="mid">grabs<br/><a title="Sort Descending" href="{$orderbygrabs_desc}"><img src="{$smarty.const.WWW_TOP}/../themes_shared/images/sorting/arrow_down.gif" alt="" /></a><a title="Sort Ascending" href="{$orderbygrabs_asc}"><img src="{$smarty.const.WWW_TOP}/../themes_shared/images/sorting/arrow_up.gif" alt="" /></a></th>
	...
	<tr class="{cycle values=",alt"}">
		...
		<td class="mid"><a title="{if $user.notes|count_characters > 0}View{else}Add{/if} Notes" href="{$smarty.const.WWW_TOP}/user-edit.php?id={$user.id}#notes"><img src="{$smarty.const.WWW_TOP}/../themes_shared/images/icons/{if $user.notes|count_characters > 0}note_edit.png{else}note_add.png{/if}" alt="" /></a></td>
		<td>{$user.rolename}</td>
