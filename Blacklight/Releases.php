...
class Releases
{
     ...
     */
    public function getBrowseRange($cat, $start, $num, $orderBy, $maxAge = -1, array $excludedCats = [], $groupName = -1, $minSize = 0): LengthAwarePaginator
    {
        ...
        $qry = Release::query()
            ->fromSub(function ($query) use ($cat, $maxAge, $excludedCats, $groupName, $minSize, $orderBy, $start, $num) {
                ...
                if (\count($excludedCats) > 0) {
                    $query->whereNotIn('releases.categories_id', $excludedCats);
                }
                if ($groupName !== -1) {
                ...
                if ($minSize > 0) {
                    $query->where('releases.size', '>=', $minSize);
                }
    ...
    public function search($searchName, $usenetName, $posterName, $fileName, $groupName, $sizeFrom, $sizeTo, $hasNfo, $hasComments, $daysNew, $daysOld, $offset = 0, $limit = 1000, $orderBy = '', $maxAge = -1, array $excludedCats = [], $type = 'basic', array $cat = [-1], $minSize = 0): array
    {
        ...
        $catQuery = '';
        if ($type === 'basic') {
            $catQuery = Category::getCategorySearch($cat, '', false);
    ...
    public function tvSearch(
        ...
        $whereSql = sprintf(
            ...
            Category::getCategorySearch($cat, '', false),
            ($maxAge > 0 ? sprintf('AND r.postdate > NOW() - INTERVAL %d DAY', $maxAge) : ''),
            ($minSize > 0 ? sprintf('AND r.size >= %d', $minSize) : '')
        );
