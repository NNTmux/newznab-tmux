...
class ProcessAdditional
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $this->_nameFixer = $options['NameFixer'] ?? new NameFixer([
                'Echo' => $this->_echoCLI,
                'Categorize' => $this->_categorize,
            ]);
        ...
        if (Settings::settingValue('apps..timeoutpath') !== '' && (int) Settings::settingValue('..timeoutseconds') > 0) {
            $this->_killString = ('"'.Settings::settingValue('apps..timeoutpath').'" --foreground --signal=KILL '.Settings::settingValue('..timeoutseconds').' "');
        }

        $this->_showCLIReleaseID = (PHP_BINARY.' '.__DIR__.DS.'ProcessAdditional.php ReleaseID: ');

        // Maximum amount of releases to fetch per run.
        $this->_queryLimit = (Settings::settingValue('..maxaddprocessed') !== '') ? (int) Settings::settingValue('..maxaddprocessed') : 25;

        // Maximum message ID's to download per file type in the NZB (video, jpg, etc).
        $this->_segmentsToDownload = (Settings::settingValue('..segmentstodownload') !== '') ? (int) Settings::settingValue('..segmentstodownload') : 2;

        // Maximum message ID's to download for a RAR file.
        $this->_maximumRarSegments = (Settings::settingValue('..maxpartsprocessed') !== '') ? (int) Settings::settingValue('..maxpartsprocessed') : 3;

        // Maximum RAR files to check for a password before stopping.
        $this->_maximumRarPasswordChecks = (Settings::settingValue('..passchkattempts') !== '') ? (int) Settings::settingValue('..passchkattempts') : 1;
        ...
        $this->_processPasswords = (((int) Settings::settingValue('..checkpasswordedrar') !== 0) && Settings::settingValue('apps..unrarpath') !== '');
    ...
    protected function _clearMainTmpPath(): void
    {
        if ($this->_mainTmpPath !== '') {
            $this->_recursivePathDelete($this->_mainTmpPath, // These are folders we don't want to delete.
                [
    ...
    {
        $releasesQuery = Release::query()->where('releases.nzbstatus', '=', 1)->whereBetween('releases.passwordstatus', [
                -6,
                -1,
            ])->where('releases.haspreview', '=', -1)->where('categories.disablepreview', '=', 0);
        if ($this->_maxSize > 0) {
        ...
        $releasesQuery->select([
            ...
            'releases.searchname',
            'releases.predb_id',
            'categories.disablepreview',
        ])->leftJoin('categories', 'categories.id', '=', 'releases.categories_id')->orderBy('releases.passwordstatus')->orderByDesc('releases.postdate')->limit($this->_queryLimit);
    ...
    protected function _processReleases(): void
    {
        foreach ($this->_releases as $this->_release) {
            $this->_echo(PHP_EOL.'['.$this->_release->id.']['.human_filesize($this->_release->size, 1).']', 'primaryOver');
    ...
    protected function _processNZBContents(): int
    {
        ...
        foreach ($this->_nzbContents as $this->_currentNZBFile) {
            ...
            if (! $this->_NZBHasCompressedFile && preg_match('/\.(part\d+|[r|z]\d+|rar|0+|0*10?|zipr\d{2,3}|zipx?)(\s*\.rar)*($|[ ")\]-])|"[a-f0-9]{32}\.[1-9]\d{1,2}".*\(\d+\/\d{2,}\)$/i', $this->_currentNZBFile['title'])) {
    ...
    protected function _getAudioInfo($fileLocation, $fileExtension): bool
    {
        ...
        $rQuery = Release::query()->where('proc_pp', '=', 0)->where('id', $this->_release->id)->select([
            'searchname',
            'fromname',
            'categories_id',
        ])->first();
        ...
        if ($rQuery === null || ! preg_match(sprintf('/%d\d{3}|%d|%d|%d/', $musicParent[0], Category::OTHER_MISC, Category::MOVIE_OTHER, Category::TV_OTHER), $rQuery->id)) {
            return false;
        }
                    ...
                    foreach ($xmlArray->getAudios() as $track) {
                        if ($track->get('album') !== null && $track->get('performer') !== null) {
                            if ((int) $this->_release->predb_id === 0 && config('nntmux.rename_music_mediainfo')) {
                                ...
                                $release->update([
                                    'searchname' => $newTitle,
                                    'categories_id' => $newCat['categories_id'],
                                    'iscategorized' => 1,
                                    'isrenamed' => 1,
                                    'proc_pp' => 1,
                                ]);
    ...
    protected function _processJPGMessageIDs(): void
    {
        ...
        if (! $this->_foundJPGSample && ! empty($this->_JPGMessageIDs)) {
            ...
            if ($jpgBinary !== false) {
                ...
                $this->_foundJPGSample = ($this->_releaseImage->saveImage($this->_release->guid.'_thumb', $this->tmpPath.'samplepicture.jpg', $this->_releaseImage->jpgSavePath, 650, 650) === 1);
    ...
    protected function _processNZBCompressedFiles($reverse = false): void
    {
        ...
        foreach ($this->_nzbContents as $nzbFile) {
            ...
            if (! preg_match('/\.(part\d+|[r|z]\d+|rar|0+|0*10?|zipr\d{2,3}|zipx?)(\s*\.rar)*($|[ ")\]-])|"[a-f0-9]{32}\.[1-9]\d{1,2}".*\(\d+\/\d{2,}\)$/i', $nzbFile['title'])) {
                continue;
            }
            ...
            if (isset($file['name'])) {
                ...
                if ($this->_extractUsingRarInfo) {
                    ...
                    if (isset($file['compressed']) && (int) $file['compressed'] === 0) {
                        File::put($this->tmpPath.random_int(10, 999999).'_'.$fileName, $this->_archiveInfo->getFileData($file['name'], $file['source']));
    ...
    protected function _addFileInfo(&$file): void
    {
        ...
        if (! isset($file['error']) && isset($file['source']) && ! preg_match($this->_supportFileRegex.'|part\d+|[r|z]\d{1,3}|zipr\d{2,3}|\d{2,3}|zipx|zip|rar)(\s*\.rar)?$/i', $file['name'])) {
             ...
             */
            if ($this->_addedFileInfo < 11 && ReleaseFile::query()->where([
                    'releases_id' => $this->_release->id,
                    'name' => $file['name'],
                    'size' => $file
                    ['size'],
                ...
                if (ReleaseFile::addReleaseFiles($this->_release->id, $file['name'], $file['size'], $file['date'], $file['pass'], '', $file['crc32'] ?? '')) {
                    ...
                    if (preg_match('/alt\.binaries\.movies($|\.divx$)/', $this->_releaseGroupName) && preg_match('/[\/\\\\]Codec[\/\\\\]Setup\.exe$/i', $file['name'])) {
                        if (config('app.debug') === true) {
    ...
    protected function _siftPAR2Info($fileLocation): void
    {
        ...
        $foundName = true;
        if ((int) $releaseInfo->proc_pp === 0 && config('nntmux.rename_par2') && \in_array((int) $this->_release->categories_id, Category::OTHERS_GROUP, false)) {
            $foundName = false;
        }
        ...
        foreach ($files as $file) {
            ...
            if ($this->_addPAR2Files) {
                if ($filesAdded < 11 && ReleaseFile::query()->where([
                        'releases_id' => $this->_release->id,
                        'name' => $file['name'],
                    ])->first() === null) {
    ...
    {
        // Try to resize/move the image.
        $this->_foundJPGSample = ($this->_releaseImage->saveImage($this->_release->guid.'_thumb', $fileLocation, $this->_releaseImage->jpgSavePath, 650, 650) === 1);
    ...
    protected function _finalizeRelease(): void
    {
        ...
        if (! $this->_releaseHasPassword && $this->_NZBHasCompressedFile && $releaseFilesCount === 0) {
            $query = sprintf('UPDATE releases
				SET passwordstatus = passwordstatus - 1, rarinnerfilecount = %d %s %s %s
				WHERE id = %d', $releaseFilesCount, $iSQL, $vSQL, $jSQL, $this->_release->id);
            ...
            $query = sprintf('UPDATE releases
				...
				WHERE id = %d', ($this->_processPasswords ? $this->_passwordStatus : Releases::PASSWD_NONE), $releaseFilesCount, $iSQL, $vSQL, $jSQL, $this->_release->id);
        }
