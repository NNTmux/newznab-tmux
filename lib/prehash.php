...
Class PreHash
{
	// If you wish to not get PRE from one of these sources, set it to false.UsenetCrawler is set to false by default.
	...
	/**
	 * Retrieve pre info from PreHash sources and store them in the DB.
	 *
	...
	public function updatePre()
	{
		...
		if ((int)$newestRel['adddate'] < (time() - 600)) {
			...
			if ($this->echooutput) {
				echo $this->c->header("Retrieving titles from prehash sources.");
			}
	...
	/**
	 * Attempts to match prehash titles / NFOs to releases.
	 *
	 * @param $nntp
	 */
	public function checkPre($nntp)
	{
		...
		if ($this->echooutput) {
			echo $this->c->header(
				'Matched ' . number_format(($matched > 0) ? $matched : 0) . ' prehash titles to release search names.'
			);
		}
		...
		if ($this->echooutput) {
			echo $this->c->header(
				"\nAdded " . number_format(($nfos > 0) ? $nfos : 0) . ' missing NFOs from prehash sources.'
			);
		}
	}
				...
				foreach ($matches as $match) {
					foreach ($match as $m) {
						if (preg_match(
							...
							$result['source'] = 'prelist';
							$result['date'] = strtotime($result['date']);
							$result['nuked'] = ($result['nuked'] === 'UNNUKED' ? PreHash::PRE_UNNUKED : PreHash::PRE_NUKED);
							$this->verifyPreData($result);
						}
					}
				}
					...
					foreach ($matches as $match) {
						foreach ($match as $m) {
							if (preg_match('/time"\s*data="(?P<time>\d+)".+?adult">(?P<cat1>.+?)<\/a>.+?child">(?P<cat2>.+?)<\/a>.+?title"\s*href=".+?">(?P<title>.+?)<\/a>(.+?Nuked:\s*(?P<nuked>.+?)">)?/i', $m, $result)) {
								...
								$result['nuked'] = (isset($result['nuked']) && !empty($result['nuked']) ? PreHash::PRE_NUKED : PreHash::PRE_NONUKE);
				...
				foreach ($m as $match) {
					if (preg_match('/left">\s*(?P<date>.+?)\s*<\/td.+?left">\s*(.*?<u>)?(?P<title>.+?)(<\/.+>?<.+?>(?P<nuke>.+)?<\/.+>)?(<\/u><\/a>.+?)?\s*<\/td.+?href.+?">\s*(?P<category>.+?)\s*<\/a.+?mid">\s*((?P<size>\d+(\.\d+)?[KMGT]?B)(\/(?P<files>\d+F))?|--)\s*<\/td/s', $match, $matches2)) {
						...
						if (isset($matches2['nuke']) && preg_match('/(\S+)\)/', $matches2['nuke'], $nuked)) {
							$matches2['nuked'] = PreHash::PRE_NUKED;
	...
	/**
	 * @note Update a single release as it's created.
	 * @param $cleanerName
	 * @param $releaseID
	 *
	 * @return bool
	...
	/**
	 * @note When a searchname is the same as the title, tie it to the prehash. Try to update the categoryID at the same time.
	 * @return int
	 */
	...
	/**
	 * @note Look if the release is missing an nfo.
	 * @param $nntp
	 *
	 * @return int
	 */
	public function matchNfo($nntp)
	{
		...
		$res = $db->queryDirect('SELECT r.ID, p.nfo, p.title, r.completion, r.guid, r.groupID FROM releases r INNER JOIN prehash p ON r.prehashID = p.ID WHERE r.nfostatus != 1 AND r.releasenfoID != 1 AND p.nfo IS NOT NULL LIMIT 100');
		$total = $res->rowCount();
		if ($total > 0) {
	...
	/**
	 * @note Returns a single row for a release.
	 * @param $preID
	 *
	 * @return array
	 */
