...
class TvAnger
{
	...
	public function fetchShowQuickInfo($show, array $options = array())
	{
		...
		$fp = fopen($url, "r", false, stream_context_create($this->utility->streamSslContextOptions()));
		if ($fp) {
			while (!feof($fp)) {
				$line = fgets($fp, 1024);
	...
	public function updateSchedule()
	{
		...
		foreach ($countries as $country) {
			...
			$sched = $this->utility->getURL(['url' => $this->xmlFullScheduleUrl . $country['country']]);
	...
	public function getEpisodeInfo($rageID, $series, $episode)
	{
		...
		$xml = $this->utility->getUrl(['url' => $this->xmlEpisodeInfoUrl . "&sid=" . $rageID . "&ep=" . $series . "x" . $episode]);
		if ($xml !== false) {
			...
			$arrXml = $this->utility->objectsIntoArray($xmlObj);
			if (is_array($arrXml)) {
	...
	public function getRageInfoFromPage($rageID)
	{
		...
		$page = $this->utility->getUrl(['url' => $this->showInfoUrl . $rageID]);
		$matches = '';
		if ($page !== false) {
			// Description.
	...
	public function getRageInfoFromService($rageID)
	{
		...
		$xml = $this->utility->getUrl(['url' => $this->xmlShowInfoUrl . $rageID]);
		if ($xml !== false) {
			$arrXml = $this->utility->objectsIntoArray(simplexml_load_string($xml));
			if (is_array($arrXml)) {
	...
	public function updateEpInfo($show, $relid)
	{
		...
		$this->pdo->queryExec(sprintf("UPDATE releases SET seriesfull = %s, season = %s, episode = %s, tvairdate = %sWHERE ID = %d", $this->pdo->escapeString($show['seriesfull']), $this->pdo->escapeString($show['season']), $this->pdo->escapeString($show['episode']), $tvairdate, $relid));
	}
	...
	public function updateRageInfo($rageID, $show, $tvrShow, $relid)
	{
		...
		if (isset($rInfo['imgurl']) && !empty($rInfo['imgurl'])) {
			$img = $this->utility->getUrl(['url' => $rInfo['imgurl']]);
			if ($img !== false) {
	...
	public function getRageMatch($showInfo)
	{
		...
		// Full search gives us the akas.
		$xml = $this->utility->getUrl(['url' => $this->xmlFullSearchUrl . urlencode(strtolower($title))]);
		if ($xml !== false) {
			$arrXml = @$this->utility->objectsIntoArray(simplexml_load_string($xml));
