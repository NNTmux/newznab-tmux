...
//Notice: This script is adapted from nZEDb for newznab by DariusIII
...
foreach ($groups as $group) {
	...
	$db->exec(sprintf("UPDATE groups SET first_record = %s, last_record = %s WHERE ID = %d", $db->escapeString($bfartnum), $db->escapeString($currartnum), $group['ID']));
...
foreach ($arr as &$value) {
	$rel = $db->exec("TRUNCATE TABLE $value");
	if ($rel !== false) {
...
function daytopost($nntp, $group, $days, $debug = true, $bfcheck = true)
{
	...
	$firstDate = $functions->postdate($data['first'], $data);
	$lastDate = $functions->postdate($data['last'], $data);
	...
	while (daysOld($dateofnextone) < $days) {
		while (($tmpDate = $functions->postdate(($upperbound - $interval), $data)) > $goaldate) {
		...
		$dateofnextone = $functions->postdate(($upperbound - 1), $data);
		while (!$dateofnextone) {
			$dateofnextone = $functions->postdate(($upperbound - 1), $data);
		}
	}
