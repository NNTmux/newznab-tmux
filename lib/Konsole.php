...
class Konsole
{
	...
	public function getConsoleRange($cat, $start, $num, $orderby, $maxage = -1, $excludedcats = array())
	{
		...
		return $db->query(sprintf("SELECT GROUP_CONCAT(r.IDORDER BY r.postdate DESC SEPARATOR ',') AS grp_release_id, "
					...
					. "WHERE con.cover = 1 AND con.title != '' AND "
...
// Basically the XBLA names contain crap, this is to reduce the title down far enough to be usable.
	...
	{
		$db = $this->db;
		$res = $db->queryDirect(sprintf('SELECT searchname, ID FROM releases WHERE %s consoleinfoID IS NULL AND categoryID BETWEEN 1000 AND 1999 ORDER BY postdate DESC LIMIT %d', $this->renamed, $this->gameqty));
