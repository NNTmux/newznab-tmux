...
{foreach from=$releases item=release}
	<item>
		...
		<link>{$serverroot}{if $dl=="1"}getnzb{else}details{/if}
		/{$release.guid}{if $dl=="1"}.nzb&amp;i={$uid}&amp;r={$rsstoken}{/if}{if $del=="1"}&amp;del=1{/if}</link>
			...
			<div>
				...
				<ul>
					...
					<li>Attributes: Category - <a
								href="{$serverroot}browse?t={$release.categoryID}">{$release.category_name}</a></li>
					...
					<li>
						Password: {if $release.passwordstatus == 0}None{elseif $release.passwordstatus == 2}Passworded Rar Archive{elseif $release.passwordstatus == 1}Contains Cab/Ace/RAR Archive{else}Unknown{/if}</li>
					...
					{if $release.parentCategoryID == 2000}
						{if $release.imdbID != ""}
							<li>Imdb Info:
								<ul>
									<li>IMDB Link: <a
												href="http://www.imdb.com/title/tt{$release.imdbID}/">{$release.imdbtitle|escape:"htmlall"}</a>
									</li>
									{if $release.rating != ""}
										<li>Rating: <span
												style="font-weight:bold; color:{if $release.rating < 4}#FF0000{elseif $release.rating < 7}#FFAA2A{else}#2AFF00{/if};">{$release.rating|escape:"htmlall"}</span>
										</li>{/if}
									{if $release.plot != ""}
										<li>Plot: {$release.plot|escape:"htmlall"}</li>{/if}
									{if $release.year != ""}
										<li>Year: {$release.year|escape:"htmlall"}</li>{/if}
									{if $release.genre != ""}
										<li>Genre: {$release.genre|replace:"|":", "|escape:"htmlall"}</li>{/if}
									{if $release.director != ""}
										<li>Director: {$release.director|replace:"|":", "|escape:"htmlall"}</li>{/if}
									{if $release.actors != ""}
										<li>Actors: {$release.actors|replace:"|":", "|escape:"htmlall"}</li>{/if}
								</ul>
							</li>
						{/if}
					{/if}
					...
					{if $release.parentCategoryID == 3000}
						{if $release.musicinfoID > 0}
							<li>Music Info:
								<ul>
									{if $release.mu_url != ""}
										<li>Amazon: <a href="{$release.mu_url}">{$release.mu_title|escape:"htmlall"}</a>
										</li>{/if}
									{if $release.mu_artist != ""}
										<li>Artist: {$release.mu_artist|escape:"htmlall"}</li>{/if}
									{if $release.mu_genre != ""}
										<li>Genre: {$release.mu_genre|escape:"htmlall"}</li>{/if}
									{if $release.mu_publisher != ""}
										<li>Publisher: {$release.mu_publisher|escape:"htmlall"}</li>{/if}
									{if $release.year != ""}
										<li>Released: {$release.mu_releasedate|date_format}</li>{/if}
									{if $release.mu_review != ""}
										<li>Review: {$release.mu_review|escape:"htmlall"}</li>{/if}
					...
					{if $release.parentCategoryID == 1000}
						{if $release.consoleinfoID > 0}
							<li>Console Info:
								<ul>
									{if $release.co_url != ""}
										<li>Amazon: <a href="{$release.co_url}">{$release.co_title|escape:"htmlall"}</a>
										</li>{/if}
									{if $release.co_genre != ""}
										<li>Genre: {$release.co_genre|escape:"htmlall"}</li>{/if}
									{if $release.co_publisher != ""}
										<li>Publisher: {$release.co_publisher|escape:"htmlall"}</li>{/if}
									{if $release.year != ""}
										<li>Released: {$release.co_releasedate|date_format}</li>{/if}
									{if $release.co_review != ""}
										<li>Review: {$release.co_review|escape:"htmlall"}</li>{/if}
								</ul>
							</li>
						{/if}
					{/if}
					...
					{if $release.categoryID == 7020}
						{if $release.bookinfoID > 0}
							<li>Console Info:
								<ul>
									{if $release.bo_author != ""}
										<li>Author: <a
												href="{$serverroot}books?author={$release.bo_author|escape:"url"}">{$release.bo_author|escape:"htmlall"}</a>
										</li>{/if}
									{if $release.bo_url != ""}
										<li>Amazon: <a href="{$release.bo_url}">{$release.bo_title|escape:"htmlall"}</a>
										</li>{/if}
									{if $release.bo_publisher != ""}
										<li>Publisher: {$release.bo_publisher|escape:"htmlall"}</li>{/if}
									{if $release.bo_publishdate != ""}
										<li>Publish Date: {$release.bo_publishdate|date_format}</li>{/if}
									{if $release.bo_review != ""}
										<li>Review: {$release.bo_review|escape:"htmlall"}</li>{/if}
								</ul>
							</li>
						{/if}
					{/if}
