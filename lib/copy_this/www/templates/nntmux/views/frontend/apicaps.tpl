...
{/foreach}
	...
	<groups>
{foreach from=$grps item=grp}		<group id="{$grp.ID}" name="{$grp.name|escape:html}"{if $grp.description != ""} description="{$grp.description|escape:html}"{/if} lastupdate="{$grp.last_updated|phpdate_format:"DATE_RSS"}"/>
{/foreach}
	</groups>

	<genres>
{foreach from=$genres item=genre}		<genre id="{$genre.ID}" categoryid="{$genre.type}" name="{$genre.title|escape:html}"/>
{/foreach}
	</genres>	
</caps>
