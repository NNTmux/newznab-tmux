...
$cfg->configCheck = is_writable(NN_WWW);
if ($cfg->configCheck === false) {
	$cfg->configCheck = is_file(NN_WWW);
...
if (file_exists(NN_WWW . 'config.php') && is_readable(NN_WWW . 'config.php')) {
	$tmpCfg = file_get_contents(NN_WWW . 'config.php');
