...
if ($page->isPostBack()) {
	...
	if (!$cfg->error) {
		...
		$pdo = new DB();
		$sql1 = sprintf("UPDATE site SET value = %s WHERE setting = 'nzbpath'", $pdo->escapeString($cfg->NZB_PATH));
		$sql2 = sprintf("UPDATE site SET value = %s WHERE setting = 'tmpunrarpath'", $pdo->escapeString($cfg->UNRAR_PATH));
		$sql3 = sprintf("UPDATE site SET value = %s WHERE setting = 'coverspath'", $pdo->escapeString($cfg->COVERS_PATH));
