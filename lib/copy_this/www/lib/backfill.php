	...
	function backfillGroup($groupArr, $backfillDate=null, $regexOnly=false)
	{
		...
		if ($nntp->doConnect(5, false, true))
		{
			...
			if ($first_record_postdate != "")
				$db->exec(sprintf("update groups SET first_record_postdate = FROM_UNIXTIME(".$first_record_postdate."), last_updated = now() WHERE ID = %d", $groupArr['ID']));  //Set group's first postdate
