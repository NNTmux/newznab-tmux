...
{	
	...
	public function update($id, $uname, $email, $grabs, $role, $notes, $invites, $movieview, $musicview, $consoleview, $bookview, $saburl=false, $sabapikey=false, $sabpriority=false, $sabapikeytype=false, $nzbvortexServerUrl = false, $nzbvortexApiKey = false, $nzbgetusername = false, $nzbgetpassword = false, $nzbgeturl = false)
	{			
		$db = new DB();
        ...
        if ($nzbgetusername !== false)
            $sql[] = sprintf("nzbgetusername = '%s", $nzbgetusername);

        if ($nzbgetpassword !== false)
            $sql[] = sprintf("nzbgetpassword = '%s", $nzbgetpassword);

        if ($nzbgeturl !== false)
            $sql[] = sprintf("nzbgeturl = '%s", $nzbgeturl);
