...
if (isset($argv[1]) && ($argv[1] == "true" || $argv[1] == "drop")) {
	...
	if ($tablepergroup == 1) {
		...
		foreach ($tables as $row) {
			$tbl = $row['Name'];
			if (preg_match(preg_match('/collections_\d+/', $tbl) || '/binaries_\d+/', $tbl) || preg_match('/parts_\d+/', $tbl) || preg_match('/partrepair_\d+/', $tbl) || preg_match('/\d+_collections/', $tbl) || preg_match('/\d+_binaries/', $tbl) || preg_match('/\d+_parts/', $tbl) || preg_match('/\d+_partrepair_\d+/', $tbl)) {
				if ($argv[1] == "drop") {
