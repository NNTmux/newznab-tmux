        ...
        <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST" class="p-6">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-gray-100">{{ old('description', $promotion->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                <x-button-link href="{{ route('admin.promotions.index') }}" variant="secondary">Cancel</x-button-link>
                <x-button type="submit" icon="fas fa-save">Update Promotion</x-button>
            </div>
        </form>
    </div>
</div>
