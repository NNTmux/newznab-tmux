                ...
                @if(!empty($catname) && is_object($catname) && !empty($catname->parent))
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ url('/browse/' . $catname->parent->title) }}" class="text-gray-700 hover:text-blue-600">{{ $catname->parent->title }}</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
                            <a href="{{ url('/browse/' . $catname->title) }}" class="text-gray-700 hover:text-blue-600">{{ $catname->title }}</a>
                        </div>
                    </li>
                @else
                    <li>
                ...
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                        <div class="text-sm">
                            ...
                            <a href="{{ url('/browse/Console/' . ($categorytitle ?? '')) }}" class="text-blue-600 hover:text-blue-800">List</a></span>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex gap-1">
                                ...
                                @if(isset($isadmin) && $isadmin)
                                    ...
                                    <button type="button" class="nzb_multi_operations_delete px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition text-sm">Delete</button>
                                @endif
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="grid grid-cols-12 gap-4">
                                    ...
                                    <div class="col-span-12 sm:col-span-8">
                                        ...
                                        <h5 class="text-lg font-semibold mb-2">
                                            <a class="text-gray-800 hover:text-blue-600" href="{{ url('/details/' . $result->guid) }}">
                                                {{ $result->title }}
                                            </a>
                                        </h5>
                                                ...
                                                <div class="flex items-start">
                                                    ...
                                                    <div class="flex flex-wrap gap-1">
                                                        @foreach(explode(', ', $result->genre) as $genre)
                                                            <span class="px-2 py-0.5 bg-gray-100 text-gray-700 rounded text-xs">{{ $genre }}</span>
                                                        @endforeach
                                                    </div>
                                                </div>
                                            @endif
                                        ...
                                        <div class="bg-gray-50 rounded-lg p-3 border border-gray-200">
                                            ...
                                            <div class="flex flex-wrap gap-2" id="guid{{ $result->guid }}">
                                                <a class="px-3 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm inline-flex items-center"
                                                   title="Download NZB"
                                                   href="{{ url('/getnzb?id=' . $result->guid) }}"
                                                   onclick="showToast('Downloading NZB...', 'success')">
                                                    <i class="fa fa-cloud-download mr-1"></i>
                                                    <span class="px-1.5 py-0.5 bg-white text-gray-800 text-xs rounded ml-1">{{ $result->grabs }}</span>
                                                </a>

                                                <a class="px-3 py-1.5 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 text-sm inline-flex items-center"
                                                   href="{{ url('/details/' . $result->guid . '#comments') }}">
                                                    <i class="fa fa-comment-o mr-1"></i>
                                                    <span class="px-1.5 py-0.5 bg-white text-gray-800 text-xs rounded ml-1">{{ $result->comments }}</span>
                                                </a>

                                                <a href="#" class="add-to-cart px-3 py-1.5 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 text-sm"
