														...
														<a title="View details"
														   href="{$smarty.const.WWW_TOP}/details/{$result.grp_release_guid}">
															<img src="{$smarty.const.WWW_TOP}/covers/book/{if $result.cover == 1}{$result.bookinfo_id}.jpg{else}{$smarty.const.WWW_ASSETS}/images/no-cover.png{/if}"
																 ...
																 width="140" border="0"
																 alt="{$result.author|escape:"htmlall"} - {$result.title|escape:"htmlall"}"/>{if isset($result.grp_release_failed) && $result.grp_release_failed > 0} <i class="zmdi zmdi-alarm" style="color: red" title="This release has failed to download for some users"></i>{/if}
														</a>
