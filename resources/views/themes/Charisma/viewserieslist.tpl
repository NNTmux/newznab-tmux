...
<p>
	<b>Jump to</b>:
	&nbsp;[ {if $seriesletter == '0-9'}<b><u>{/if}<a href="{$smarty.const.WWW_TOP}/series?id=0-9">0-9</a>{if $seriesletter == '0-9'}</u></b>{/if}
	{foreach $seriesrange as $range}
	{if $range == $seriesletter}<b><u>{/if}<a href="{$smarty.const.WWW_TOP}/series?id={$range}">{$range}</a>{if $range == $seriesletter}</u></b>{/if}
	{/foreach}]
</p>
<div class="btn-group">
...
{if $serieslist|@count > 0}
	...
	<div class="col-md-12 pull-right">
		...
		{foreach $serieslist as $sletter => $series}
			...
			{foreach $series as $s}
				<tr>
					<td><a class="title" title="View series" href="{$smarty.const.WWW_TOP}/series?id={$s.id}">{if !empty($s.title)}{$s.title|escape:"htmlall"}{/if}</a>{if $s.prevdate != ''}<br /><span class="label label-info">Last: {$s.previnfo|escape:"htmlall"} aired {$s.prevdate|date_format}</span>{/if}</td>
