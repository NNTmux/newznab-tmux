											...
											<div class="row small-gutter-left">
												<div class="col-md-3 small-gutter-left">
													...
													<a href="{$smarty.const.WWW_TOP}/xxx/?id={$result.id}"><img
																...
																src="{if $result.cover == 1}{$smarty.const.WWW_TOP}/covers/xxx/{$result.id}-cover.jpg{else}{$smarty.const.WWW_TOP}/shared/images/no-cover.png{/if}"
																width="140" border="0"
													...
													{if !empty($mfailed[$m@index])}
														<span class="btn btn-default btn-xs" title="This release has failed to download for some users">
															<i class="zmdi zmdi-thumbs-o-up"></i> {$mgrabs[$m@index]} Grab{if {$mgrabs[$m@index]} != 1}s{/if} / <i class="zmdi zmdi-thumbs-o-down"></i> {$mfailed[$m@index]}Failed Download{if {$mfailed[$m@index]} > 1}s{/if}</span>
													{/if}
												</div>
