	...
	<table style="width:100%;margin-top:10px;"
		...
		{foreach $anidblist as $anidb}
			<tr class="{cycle values=",alt"}">
				...
				<td><a title="Edit"
					   href="{{url("/admin/anidb-edit?id={$anidb->anidbid}")}}")>{$anidb->title|escape:"htmlall"}</a>
				</td>
				<td class="mid"><a title="Delete this AniDB entry"
								   href="{{url("/admin/anidb-delete?id={$anidb->anidbid}")}}">delete</a> | <a
							title="Remove this anidbid from all releases"
							href="{{url("/admin/anidb-remove?id={$anidb->anidbid}")}}">remove</a></td>
			</tr>
		{/foreach}
