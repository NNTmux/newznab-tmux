<div class="card">
		...
		<div class="mb-4">
		    <h5 class="border-bottom pb-2 mb-3">Import NZBs Settings</h5>
		</div>

		<div class="row mb-4">
		    <div class="col-lg-3 col-md-4">
		        <label for="import" class="form-label fw-bold">Import NZBs:</label>
		    </div>
		    <div class="col-lg-9 col-md-8">
		        <div class="input-group">
		            <span class="input-group-text"><i class="fa fa-file-import"></i></span>
		            <select id="import" name="import" class="form-select">
		                {html_options values=$import_ids output=$import_names selected=$site->import}
		            </select>
		        </div>
		        <small class="text-muted">Choose to run import nzb script true/false. This can point to a single folder with multiple subfolders on just the one folder. If you run this threaded, it will run 1 folder per thread.</small>
		    </div>
		</div>

		<div class="row mb-4">
		    <div class="col-lg-3 col-md-4">
		        <label for="nzbs" class="form-label fw-bold">NZBs Folder:</label>
		    </div>
		    <div class="col-lg-9 col-md-8">
		        <div class="input-group">
		            <span class="input-group-text"><i class="fa fa-folder"></i></span>
		            <input id="nzbs" name="nzbs" class="form-control" type="text" value="{$site->nzbs}"/>
		        </div>
		        <small class="text-muted">Set the path to the NZB dump you downloaded from torrents, this is the path to bulk files folder of NZBs. This is by default, recursive and threaded. You set the threads in Advanced Settings.</small>
		    </div>
		</div>

		<div class="row mb-4">
		    <div class="col-lg-3 col-md-4">
		        <label for="import_count" class="form-label fw-bold">Import NZBs per Process:</label>
		    </div>
		    <div class="col-lg-9 col-md-8">
		        <div class="input-group">
		            <span class="input-group-text"><i class="fa fa-list-ol"></i></span>
		            <input id="import_count" name="import_count" class="form-control" type="text" value="{$site->import_count}"/>
		        </div>
		        <small class="text-muted">How many NZB files to import per process.</small>
		    </div>
		</div>

		<div class="row mb-4">
		    <div class="col-lg-3 col-md-4">
		        <label for="import_timer" class="form-label fw-bold">Import NZBs Sleep Timer:</label>
		    </div>
		    <div class="col-lg-9 col-md-8">
		        <div class="input-group">
		            <span class="input-group-text"><i class="fa fa-clock-o"></i></span>
		            <input id="import_timer" name="import_timer" class="form-control" type="text" value="{$site->import_timer}"/>
		            <span class="input-group-text">seconds</span>
		        </div>
		        <small class="text-muted">The time to sleep from the time the loop ends until it is restarted, in seconds.</small>
		    </div>
		...
		<div class="row mb-4">
		    ...
		    <div class="col-lg-9 col-md-8">
		        <div class="card bg-light">
		            <div class="card-body">
		                <p class="mb-0">This will import all NZBs in the given path. If in your path you have NZBs in the root folder and subfolders (regardless of NZBs inside), threaded scripts will ignore all NZBs in the root path. Then each subfolder is threaded.</p>
		            </div>
		        </div>
		    </div>
		</div>
