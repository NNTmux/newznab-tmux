			...
			{foreach from=$regex item=row}
				<tr id="row-{$row.id}" class="{cycle values=",alt"}">
					...
					<td title="Edit this regex"><a
								href="{{url("/admin/release_naming_regexes-edit?id={$row.id}")}}">Edit</a>
					</td>
					...
					{/if}
					<td title="Edit this regex"><a
								href="{{url("/admin/release_naming_regexes-edit?id={$row.id}")}}">{$row.regex|escape:html|truncate:50:"...":true}</a>
					</td>
				</tr>
			{/foreach}
		</table>
