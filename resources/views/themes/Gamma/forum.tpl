...
<table style="width:100%;" class="data highlight table" id="forumtable">
	...
	<tr class="{cycle values=",alt"}" id="guid{$result.id}">
		...
		<td>
			<a href="{$smarty.const.WWW_TOP}/forumpost/{$result.id}#last" title="{$result.updated_at}">{$result.updated_at|date_format}</a> <div class="hint">({$result.updated_at|timeago})</div>
		</td>
