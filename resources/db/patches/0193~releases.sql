...
# Triggers are recreated completely to prevent any error.
...
CREATE TRIGGER delete_search AFTER DELETE ON releases FOR EACH ROW BEGIN DELETE FROM releasesearch WHERE releaseid = OLD.id;DELETE FROM dnzb_failures WHERE guid = OLD.guid; END;$$
DELIMITER ;