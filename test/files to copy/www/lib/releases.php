...
{	
	...
	public function removeRageIdFromReleases($rageid)
	{
		...
		$ret = $res["num"];
		$db->query(sprintf("update releases set rageID = -1, seriesfull = null, season = null, episode = null where rageID = %d", $rageid));
		return $ret;
	}
	...
	public function removeAnidbIdFromReleases($anidbID)
	{
		...
		$ret = $res["num"];
		$db->query(sprintf("UPDATE releases SET anidbID = -1, episode = null, tvtitle = null, tvairdate = null where anidbID = %d", $anidbID));
		return $ret;
	}
	...
	function processReleases() 
	{
				...
				if (preg_match ($regexrow["regex"], $rowbin["name"], $matches)) 
				{
					...
					if (isset($matches['name']) && isset($matches['parts'])) 
					{
						...
						if (isset($matches['reqid'])) 
							$reqid = $db->escapeString($matches['reqid']);
                        ...
                        $sql = sprintf("update binaries set relname = replace(%s, '_', ' '), relpart = %d, reltotalpart = %d, procstat=%d, categoryID=%s, regexID=%d, reqID=%s where ID = %d",
		...
		while ($row = $db->getAssocArray($result))
		{
			...
			if ($reqIDused == "")				
				...
				$reqID = $db->escapeString($reqIDused);
			...
			{
				$db->query(sprintf("update releases set totalpart = %d, size = %s, completion = %d, GID=%s where ID = %d",  $nzbInfo->filecount, $nzbInfo->filesize, $nzbInfo->completion, $db->escapeString($nzbInfo->gid),  $relid ));
		...
		if ($page->site->partsdeletechunks > 0)
		{
			...
			$query=sprintf("SELECT parts.ID as partsID,binaries.ID as binariesID
						FROM parts
						LEFT JOIN binaries  ON binaries.ID = parts.binaryID
						WHERE binaries.procstat IN (%d,%d)
						OR binaries.dateadded < %s - INTERVAL %d HOUR LIMIT 0,%d",
						Releases::PROCSTAT_RELEASED, Releases::PROCSTAT_DUPLICATE,
			...
			while(!$done)
			{
				...
				$pID='('.implode(',',$pID).')';
				$bID='('.implode(',',$bID).')';
				$db->query("DELETE FROM parts WHERE ID IN {$pID}");
				$fr=$db->getAffectedRows();
				if($fr>0)
				{
				   $cc+=$fr;
				   $db->query("DELETE FROM binaries WHERE ID IN {$bID}");
				   $cc+=$db->getAffectedRows();
				}
				unset($pID);unset($bID);
				if($cc==$dd)
				{
		...
		else
		{
			...
			LEFT JOIN binaries ON binaries.ID = parts.binaryID 
			WHERE binaries.procstat IN (%d, %d) 
			OR binaries.dateadded < %s - INTERVAL %d HOUR", Releases::PROCSTAT_RELEASED, Releases::PROCSTAT_DUPLICATE, $db->escapeString($currTime_ori["now"]), ceil($page->site->rawretentiondays*24)));		
		}
