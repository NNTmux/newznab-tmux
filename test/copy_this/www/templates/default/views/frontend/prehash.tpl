...
<table style="width:100%;margin-bottom:10px; margin-top:5px;" class="data Sortable highlight">
	...
	{foreach from=$results item=result}
		<tr class="{cycle values=",alt"}">
			...
			{/if}
			<td class="prehash">{$result.adddate|date_format:"%Y-%m-%d %H:%M:%S"}</td>
			<td class="prehash">{$result.predate|date_format:"%Y-%m-%d %H:%M:%S"}</td>
				{if $isadmin || $ismod}
			<td class="prehash">
			...
			<td class="prehash">
				...
				{elseif {$result.category} == 'EBOOK'}
					<a class="title" title="View category Books" href="{$smarty.const.WWW_TOP}/browse?t=7000">
						{$result.category}
					</a>
