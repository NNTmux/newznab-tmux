...
class Namefixer
{
    ...
    public function fixNamesWithNfo($time, $echo, $cats, $namestatus)
    {
        ...
        $query = "SELECT nfo.releaseID as nfoID, rel.groupID, rel.categoryID, rel.searchname, uncompress(nfo) as textstring, rel.ID as releaseID from releases rel inner join releasenfo nfo on (nfo.releaseID = rel.ID) and relnamestatus = 1 and relstatus & " . DB::NFO_PROCESSED_NAMEFIXER . " = 0";

        //24 hours, other cats
        if ($time == 1 && $cats == 1)
        {
            $relres = $db->queryDirect($query." and rel.adddate > (now() - interval 6 hour) and rel.categoryID in (2020, 5050, 6070, 8010) group by rel.ID order by postdate desc");
        }
        //24 hours, all cats
        if ($time == 1 && $cats == 2)
        {
        ...
        if ($time == 2 && $cats == 1)
        {
            $relres = $db->queryDirect($query." and rel.categoryID in (2020, 5050, 6070, 8010) group by rel.ID order by postdate desc");
        }
        //all cats
        if ($time == 2 && $cats == 2)
        {
    ...
    public function fixNamesWithFiles($time, $echo, $cats, $namestatus)
    {
        ...
        $db = new DB();
        $type = "Filenames, ";
        $query = "SELECT relfiles.name as textstring, rel.categoryID, rel.searchname, rel.groupID, relfiles.releaseID as fileID, rel.ID as releaseID from releases rel inner join releasefiles relfiles on (relfiles.releaseID = rel.ID) and relnamestatus = 1";
				...
				if ($row["title"] !== $release["searchname"])
				{
					...
					if ($this->echooutput)
					{
						...
						echo"New name: ".$row["title"]."\n".
							...
							"Method:   "."prehash md5 release name: ".$row["source"]."\n"."\n";
					}
					$matched++;
				}
			}
