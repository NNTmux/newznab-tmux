...
class nameCleaning
{
 	...
 	public function releaseCleaner($subject, $groupName, $usepre = false)
    {
        ...
        preg_match_all('/([\w\(\)]+[\s\._-]([\w\(\)]+[\s\._-])+[\w\(\)]+-\w+)/', $this->subject, $matches);
		...
		if (preg_match('/^\[(\d+)\]/', $this->subject, $match) || preg_match('/^\[ (\d+) \]/', $this->subject, $match)) {
			...
			$title = $db->queryOneRow(sprintf('SELECT title, ID from prehash WHERE requestID = %d and groupID = %d ', $match[1], $groupID));
			if (isset($title['title'])) {
				$cleanerName = $title['title'];
				if (!empty($cleanerName)) {
					return array(
